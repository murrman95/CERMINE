...
/**
 * Writes BxDocument model pages to JSON format.
 *
 * @author Christopher Murray
 */

public class BxDocumentToJSONWriter {
    ...
    static {
        OUTPUT_PROPERTIES.setProperty(OutputKeys.DOCTYPE_SYSTEM, "JSON");
    ...
    /*Chris's code starting here
     *
    */

    private HashMap<String,Number> getWordData(BxZone zone){
        ArrayList<BxWord> words = new ArrayList<BxWord>();
        ArrayList<BxLine> bxLines = new ArrayList<BxLine>();
        zone.iterator().forEachRemaining(bxLines::add);
        double avgLineHeight = 0.0;
        for(BxLine bxLine : bxLines){
            avgLineHeight += bxLine.getBounds().getHeight();
            bxLine.iterator().forEachRemaining(words::add);
        }
        avgLineHeight /= bxLines.size();

        List<BxChunk> chunks = zone.getChunks();
        int italicsCounter = 0;
        int normalCounter = 0;
        int boldCounter = 0;
        for(BxWord bxWord: words){
            String fontName = bxWord.getMostPopularFontName();
            boolean normal = true;
        ...
        res.put("font_size",avgLineHeight);
        double wordCount = (double)italicsCounter + normalCounter + boldCounter;
        res.put("percent_normal",(double)normalCounter/wordCount);
        res.put("percent_bold",(double)boldCounter/wordCount);
        res.put("percent_italics",(double)italicsCounter/wordCount);
        return res;
    }

    public JsonObjectBuilder JsonZone(BxZone zone, HashMap<String,Number> toAdd){
        HashMap<String,Object> config = new HashMap<String,Object>();
        JsonBuilderFactory zoneBuilder = Json.createBuilderFactory(config);
        JsonObjectBuilder zoneObject = zoneBuilder.createObjectBuilder();
        for(Map.Entry<String,Number> entry: toAdd.entrySet()){
            String fieldName = entry.getKey();
            Number value = entry.getValue();
            zoneObject = zoneObject.add(fieldName,value.doubleValue());
        }
        BxBounds zoneBounds = zone.getBounds();

        HashMap<String,Number> wordData = getWordData(zone);
        for(Map.Entry<String,Number> entry: wordData.entrySet()){
            String fieldName = entry.getKey();
            Number value = entry.getValue();
            zoneObject = zoneObject.add(fieldName,value.doubleValue());
        }
        ...
        zoneObject = zoneObject.add("y_coord",zoneBounds.getY())
        .add("x_coord",zoneBounds.getX())
        .add("block_height",zoneBounds.getHeight())
        .add("block_width",zoneBounds.getWidth())
        .add("num_words",(long)textArray.length)
        .add("num_chars",text.length())
        .add("text",text)
        //Just getting an empty array. Hope this works
        .add("labels", zoneBuilder.createArrayBuilder())
        .add("type_prediction", zone.getLabel().name());

        return zoneObject;
    }

    public JsonArray JsonZones(List<BxPage> pages){

        HashMap<String,Object> config = new HashMap<String,Object>();
        JsonBuilderFactory zoneBuilder = Json.createBuilderFactory(config);
        JsonArrayBuilder zones = zoneBuilder.createArrayBuilder();
        ArrayList<BxPage> pageList = new ArrayList<BxPage>(pages);
        // For passing the page number, height, and width info to the JsonZone (so our schema is consistent with others).
        for(int i = 1; i <= pageList.size(); i ++){
            HashMap<String,Number> toAdd = new HashMap<String,Number>();
            BxPage page = pageList.get(i-1);
            toAdd.put("page_height",page.getBounds().getHeight());
            toAdd.put("page_width",page.getBounds().getWidth());
            toAdd.put("page_number",i);
            toAdd.put("num_pages",pageList.size());
            ArrayList<BxZone> bxZones = new ArrayList<BxZone>();
            page.iterator().forEachRemaining(bxZones::add);;

            for (int j = 0; j < bxZones.size(); j ++) {
                
                zones = zones.add(zoneBuilder.createObjectBuilder().add("paragraph_number",j+i)
                    .add("paragraph",JsonZone(bxZones.get(j),toAdd)));
            }
        }
        JsonArray res = zones.build();
        return res;
    }
    public JsonObject JsonDocument(List<BxPage> pages){
        HashMap<String,Object> config = new HashMap<String,Object>();
        JsonBuilderFactory docBuilder = Json.createBuilderFactory(config);
        JsonObject doc = docBuilder.createObjectBuilder()
            .add("paragraphs", JsonZones(new ArrayList<BxPage>(pages))).build();
        return doc;
    }


    public String write(List<BxPage> objects, Object... hints) throws Exception {
    ...
    public void write(Writer writer, List<BxPage> objects, Object... hints) throws Exception {
        try {
            JsonObject jDoc = JsonDocument(objects);
